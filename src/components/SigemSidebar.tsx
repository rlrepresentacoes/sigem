...
const SigemSidebar = ({ className }: SidebarProps) => {
  ...
  return (
    <div
      className={cn(
        'flex flex-col h-screen bg-gradient-to-b from-sigem-dark-blue via-[#1b2033] to-[#1d2239] text-white transition-all duration-300 border-r border-gray-800/30 shadow-lg',
    ...
    >
              ...
              <Tooltip>
                <TooltipTrigger asChild>
                  <Button
                    variant="ghost"
                    className={cn(
                      'w-full justify-start text-white/80 hover:text-white hover:bg-white/10 transition-colors relative group',
                      location.pathname === item.path ? 'bg-white/15 text-white font-medium' : '',
                      collapsed ? 'px-2' : 'px-3 h-11'
                    )}
                    onClick={() => navigate(item.path)}
                  >
                    <item.icon className={cn(
                      'h-5 w-5 flex-shrink-0',
                      location.pathname === item.path ? 'text-primary' : 'text-white/70 group-hover:text-white',
                      collapsed ? '' : 'mr-3'
                    )} />
                ...
                {(collapsed || item.description) && (
                  <TooltipContent 
                    side="right" 
                    className="bg-gray-900 text-white border-gray-800 flex flex-col space-y-1"
                  >
                    ...
                    {item.badge && collapsed && (
                      <Badge 
                        variant="outline" 
                        className="mt-1 bg-primary/20 text-primary border-primary/30"
                      >
                    ...
                    {item.new && collapsed && (
                      <Badge 
                        variant="outline" 
                        className="mt-1 bg-green-500/20 text-green-400 border-green-500/30"
                      >
                        Novo
                      </Badge>
                    )}
      ...
      <div className={cn(
        "border-t border-gray-700/30 p-3",
        collapsed ? "px-2" : ""
      )}>
        <div className={cn(
          "flex items-center",
            ...
            <Tooltip>
              <TooltipTrigger asChild>
                <Button
                  variant="ghost"
                  className={cn(
                    'border border-gray-700/50 bg-white/5 hover:bg-white/10 transition-colors w-full justify-center text-white/90 hover:text-white',
                    collapsed ? 'px-2 aspect-square' : 'px-3'
                  )}
                  onClick={logout}
                >
        ...
        {collapsed && (
          <div className="mt-3 flex justify-center">
            <Button
              ...
              size="icon"
              onClick={() => setCollapsed(!collapsed)}
              className="text-white/70 hover:bg-white/10 hover:text-white rounded-full aspect-square p-1.5"
            >
